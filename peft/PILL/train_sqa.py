...
from data import ScienceQADataSet, data_collate
...
def get_args_parser():
    ...
    # parser.add_argument('--resume', default='/data/zfy/output_dir/checkpoint-10.pth',
    ...
    parser.add_argument('--caption_file', type=str, default='/data/zfy/dataset/scienceQA/captions.json')
    parser.add_argument('--data_root', type=str, default='/data/zfy/dataset/scienceQA')
    parser.add_argument('--use_caption', action='store_true', help='use image captions or not')

    parser.add_argument('--do_pretrain', type=bool, default=False, 
                        help='pre-train on large scale vl instruction')

    parser.add_argument('--pretrain_model', type=str, default='/data/zfy/stich_adapter/pretrained-3.pth', help='pre-trained model path')
    # parser.add_argument('--pretrain_model', type=str, default=None, help='pre-trained model path')
    parser.add_argument('--random_select_rate', type=float, default=0, help='random_select_rate')
...
def main(args):
    ...
    model, hook_list = Llama(args, adapter_config=adapter_config)

    model = model.to(device)

    with ExitStack() as stack:
        [stack.enter_context(hook) for hook in hook_list]
