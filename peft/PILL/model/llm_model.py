...
class Attention(nn.Module):
    ...
    def forward(self, 
        ...
        if not self.args.do_pretrain:
            ...
            with open('./attn_gate/{}.csv'.format(self.layer_id), 'a+') as f:
                np.savetxt(f, torch.abs(xg[image_position[..., 0]]).float().detach().cpu().numpy(),fmt='%.4f',delimiter=',')
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], image_position=None):
        ...
        h = x + self.attention.forward(self.attention_norm(x), start_pos, freqs_cis, mask, image_position)
